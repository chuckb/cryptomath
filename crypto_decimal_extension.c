...
static void crypto_sum_step(
    ...
    if (argc != 3) {
        sqlite3_result_error(context, "crypto_sum requires 3 arguments (operand_denomination, final_denomination, operand)", -1);
        return;
    }

    // Parse the command arguments
    const unsigned char *operand_denom_str = sqlite3_value_text(argv[0]);
    const unsigned char *final_denom_str = sqlite3_value_text(argv[1]);
    const unsigned char *operand_str = sqlite3_value_text(argv[2]);
    if (!operand_denom_str || !final_denom_str) {
        sqlite3_result_null(context);
        return;
    }
    ...
    // Get the final denom
    crypto_denom_t final_denom = crypto_get_denom_for_symbol((const char*)final_denom_str);
    ...
    crypto_type_t crypto_type = crypto_denoms[final_denom].crypto_type;

    // Get the operand denom
    crypto_denom_t operand_denom = crypto_get_denom_for_symbol((const char*)operand_denom_str);
